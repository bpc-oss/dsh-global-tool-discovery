...
import { registerSourceAdapter, sourceAdapters } from './registry.mjs'

const DEFAULT_SEARXNG_INSTANCES = [
  { url: 'http://45.32.116.133:18080', priority: 'high' },
    ...
    const exaResult = await runWithRetry(
      async () => {
        try {
          ...
          if (/rate|429/i.test(msg)) return { ok: false, output: null, code: 'rate_limit' }
          if (/auth|401/i.test(msg)) return { ok: false, output: null, code: 'auth_error' }
          return { ok: false, output: null, code: 'unavailable' }
        } catch (e) {
...
export async function apply(ctx, config = {}) {
  if (Array.isArray(config.adapterWhitelist)) {
    if (config.adapterWhitelist.includes('rss')) {
      try {
        const { rssAdapter } = await import('../adapters/rss.mjs')
        registerSourceAdapter('rss', rssAdapter)
      } catch (e) {
        ctx.logger?.warn?.(`[research] rss adapter load failed: ${e.message}`)
      }
    }
    if (config.adapterWhitelist.includes('hackernews')) {
      try {
        const { hackernewsAdapter } = await import('../adapters/hackernews.mjs')
        registerSourceAdapter('hackernews', hackernewsAdapter)
      } catch (e) {
        ctx.logger?.warn?.(`[research] hackernews adapter load failed: ${e.message}`)
      }
    }
  }
  const jobs = new Map()

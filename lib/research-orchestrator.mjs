...
async function searxngSearch(query, limit) {
  for (const base of SEARXNG_INSTANCES) {
    try {
      const url = `${base}/search?q=${encodeURIComponent(query)}&format=json&language=en&pageno=1`
...
async function runResearch(query, sources, limit) {
  ...
  if (sources.includes('searxng')) {
    const items = await searxngSearch(query, limit)
    if (items.length > 0) {
      status.push({ source: 'searxng', active: true })
      sections.push(`\n## Web (SearXNG)`)
      for (const r of items) {
        sections.push(`- ${r.title || '?'}: ${r.url || '?'}`)
        if (r.content) sections.push(`  ${r.content.slice(0, 200)}`)
      }
    } else {
      status.push({ source: 'searxng', active: false, info: 'no results / instances blocked' })
    }
  }
  ...
  if (sources.includes('code') || sources.includes('github')) {
    const ghOut = await runAsync('gh', ['search', 'repos', query, '--limit', String(limit), '--json', 'name,url,description,stargazersCount'], 30000)
    if (ghOut.ok) {
      status.push({ source: 'code', active: true })
      sections.push(`\n## Code (GitHub)`)
      const parsed = safeJson(ghOut.output)
      if (Array.isArray(parsed)) {
        for (const r of parsed.slice(0, limit)) {
          sections.push(`- ${r.name || '?'}: ${r.url || '?'}`)
          if (r.description) sections.push(`  ${r.description.slice(0, 200)}`)
          if (r.stargazersCount) sections.push(`  ⭐ ${r.stargazersCount}`)
        }
      } else {
        sections.push(`  ${ghOut.output.slice(0, 500)}`)
      }
    } else {
      status.push({ source: 'code', active: false, info: ghOut.output })
    }
  }
...
export function apply(ctx) {
  ...
  ctx.effect(() => {
    const disposeStart = ctx.tools.register({
      ...
      description: 'Start a background multi-source research job. Returns a jobId immediately; use research_job_get to retrieve results. Sources: papers (litkit), web (Exa), searxng (keyless meta-search), code (GitHub), health (agent-reach).',
      timeoutMs: 10000,

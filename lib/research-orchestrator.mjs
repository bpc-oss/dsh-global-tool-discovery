...
async function runWithRetry(fn, { maxRetries = 1, delayMs = 2000 } = {}) {
  let last
...
function normalizeExaItems(parsed) {
  const texts = []
...
function normalizeSearxngItems(items) {
...
function normalizeCodeItems(items) {
  return (Array.isArray(items) ? items : []).map((r) => ({
    title: r.name || '',
    url: r.url || '',
...
async function runResearch(query, sources, limit, config) {
  ...
  if (sources.includes('web')) {
    try {
      ...
      if (exaResult && exaResult.result && !exaResult.error) {
        grouped.push({ source: 'web', active: true, results: normalizeExaItems(exaResult).slice(0, limit) })
      } else {
        errors.push({ source: 'web', active: false, status: 'unavailable', error: 'Exa MCP call failed', retryCount: 0, lastAttempt: new Date().toISOString() })
      }
    } catch (e) {
      errors.push({ source: 'web', active: false, status: 'timeout', error: String(e?.message || e), retryCount: 0, lastAttempt: new Date().toISOString() })
    }
  }
  ...
  if (sources.includes('searxng')) {
    ...
    if (items.length > 0) {
      grouped.push({ source: 'searxng', active: true, results: items })
    } else {
      errors.push({ source: 'searxng', active: false, status: 'unavailable', error: 'no results / instances blocked', retryCount: 0, lastAttempt: new Date().toISOString() })
    }
  }

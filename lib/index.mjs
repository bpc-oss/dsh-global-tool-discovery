...
import { apply as applyResearch, registerSourceAdapter, sourceAdapters } from './research-orchestrator.mjs'
import { apply as applyKnowledge } from './knowledge-ingest.mjs'
export { registerSourceAdapter, sourceAdapters }
...
export function apply(ctx, config = {}) {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    try {
      ...
      try {
        ctx.logger?.warn?.(`[dsh-global-tool-discovery] hint augmentation failed: ${String((error && error.message) || error)}`)
      } catch {
  ...
  // Mount research_orchestrator and knowledge_ingest tools
  try { applyResearch(ctx, {}); applyKnowledge(ctx, {}) } catch (e) { ctx.logger?.warn?.(`[dsh-global-tool-discovery] failed to mount sub-plugins: ${e.message}`) }
}
